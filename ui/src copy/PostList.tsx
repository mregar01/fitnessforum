...
const PostList: React.FC<PostListProps> = ({ posts, type }) => {
  return (
    <div>
      {posts.map((data) => {
        const postLink = data.title ? `/posts/${data.id}` : `/posts/${data.parentId}`;
        const postTitle =
          type === 'search' ? (
            data.title ?
              <span>
        ...
        const postTags = data.title ? data.tags : data.parentTags;
            ...
            <div className="container">
              <div className="row align-items-start">
                <div className="col-lg-2 col-4 offset-1 text-center">
                  {data.votes} votes <br />                                    
                  <span className={`border ${hasAcceptedAnswer ? 'background bg-success text-light rounded border-success' : 'border-success text-success rounded'}`}>
                    {hasAcceptedAnswer && (
                      <svg
